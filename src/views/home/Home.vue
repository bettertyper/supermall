...
<script>
  ...
  export default {
      ...
      mounted(){
        ...
        this.$bus.$on('itemImageLoad', () => {                              //监听事件总线中的itemImageLoad事件
      ...
      methods: {
        ...
        contentScroll(position){
          ...
          this.isTabFixed = position.y < -this.tabOffsetTop   //决定scroll外的tabControl是否显示
        },
        loadMore(){
          this.getHomeGoods(this.currentType);
        },
        swiperImageLoad(){                                                //应该在所有图片加载完成后，再获取offsetTop的值，保证图片位置的高度正确。通常轮播图加载完成后就行了
...
<style scoped>
  ...
  .home-wrapper{
    height: calc(100vh - 93px);                 /*或者用绝对定位，top:44px，bottom:49px就把wrapper位置固定且拉开了*/
    overflow: hidden;
  }
  .fixed{
    position: fixed;
